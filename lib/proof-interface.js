...
async function generateNewLesson(){
  ...
  const stretchWord = weightedRandom(
  ...
  const stretchWord2 = weightedRandom(possibleStretchWords  
    .filter(item => item.word != stretchWord.word),//stretch word 2 takes at a weighted random, not including existing stretch word
    item => 100 - item.mastery//for each item, you are subtracting 100 -  mastery score
    );
  ...
  console.log('stretch word2:', stretchWord2.word)
  ...
  if(Math.random() < .5){
    ...
    ${stretchWord.word},
    ${stretchWord2.word}`//2 word prompt
    console.log("2 word sentence!")
  }else{
    numWordPrompt = `The sentence MUST incorporate this word: ${stretchWord.word}`//1 word prompt
