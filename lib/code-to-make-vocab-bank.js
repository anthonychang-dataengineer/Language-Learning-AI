...
(async () => {//for some reason, Javascript requires you to make it explicit that you will run something, and wait for the the code inside async to finish, and THEN you are allowed to do other stuff
    for (let i = 0; i < 6; i++) { 
        ...
        //importedRows[i] = [];
        const filePath = `C:\\Users\\17322\\Desktop\\My Language Learning App\\HSK Word List\\hsk updated with Claude\\hsk${i+1}_with_n_frequencies.csv`
    ...
    for (let i = 1; i < 6; i++) {//iterate through each hsk, starting with hsk2
        offsetAmount = offsetAmount + wordList[i-1].length//the offset amount will be the size of the first hsk. 
    ...
    //console.log(wordList[2][0])
    let master_for_hsk1 = 70
    for (let i = 0; i < 6; i++) {
        if (i > 0){//this is simp,y to populate hsk1 so we have some words that are above th 70 threshold
            master_for_hsk1 = 0
        }
        eachVocabBank[i] = [];
